...
public class UsuarioSecurityService implements UserDetailsService {
    ...
    @Override                            //email do usuario
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        // Vai buscar no banco o usuario com o email indica em username
        Optional<Usuario> usuarioOptional = this.usuarioRepository.findByEmail(username);
        if(usuarioOptional.isEmpty()){
            throw new UsernameNotFoundException("Usuário não encontrado"); // Não encontrou o usuario com o email indicado em username
        }else{
